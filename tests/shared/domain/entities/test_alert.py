...
class Test_Alert:
    def test_alert(self):
        alert (title = "Aviso", description = "quadra molhada por conta da chuva", start_date = int(time.time()), end_date =  int(time.time()) + 7200000, severity = 2)
    
    def test_title_is_none(self):
        with pytest.raises(EntityError):
            alert(title = None, description = "quadra molhada por conta da chuva", start_date = int(time.time()), end_date =  int(time.time()) + 7200000, severity = 2)
    
    def test_title_is_not_str(self):
        with pytest.raises(EntityError):
            alert(title = 123, description = "quadra molhada por conta da chuva", start_date = int(time.time()), end_date =  int(time.time()) + 7200000, severity = 2)
    
    def test_title_first_char_is_not_uppercase(self):
        with pytest.raises(EntityError):
            alert(
                title="aviso",description="quadra molhada por conta da chuva", start_date=int(time.time()), end_date=int(time.time()) + 7200000,severity=2)

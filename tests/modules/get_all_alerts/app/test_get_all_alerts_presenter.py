...
from src.modules.get_all_alerts.app.get_all_alerts_presenter import lambda_handler
import json
...
class TestGetAlertPresenter:
    def test_get_alert_presenter(self):
        ...
        assert json.loads(response['body'])['message'] == 'Alerts were retreived successfully'
        assert len(json.loads(response['body'])['Alerts']) == 1
